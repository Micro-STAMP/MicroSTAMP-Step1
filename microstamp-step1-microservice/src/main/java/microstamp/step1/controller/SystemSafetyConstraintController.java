...
public class SystemSafetyConstraintController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<SystemSafetyConstraint> findById(@PathVariable(name = "id") UUID id) throws Step1NotFoundException {
        return new ResponseEntity<>(systemSafetyConstraintService.findById(id), HttpStatus.OK);
    }

    @GetMapping("/project/{id}")
    public ResponseEntity<List<SystemSafetyConstraint>> findByProjectId(@PathVariable(name = "id") UUID id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Void> update(@PathVariable(name = "id") UUID id, @Valid @RequestBody SystemSafetyConstraintDto systemSafetyConstraintDto) throws Step1NotFoundException {
